...
const DATE_LOCALES: Record<string, Locale> = { it, en: enUS, fr, de, es }
...
export default function ProviderChatList() {
  ...
  const loadConversations = async (uid: string) => {
    ...
    const convs = await Promise.all(missions.map(async (m) => {
      const otherUser = uid === m.client_id ? (m.provider as any) : (m.client as any)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">
          💬 {t('navChat')} {totalUnread > 0 && <span style={{color:'#1A73E8'}} className="text-xl">({totalUnread})</span>}
        </h1>
        <button onClick={() => user && loadConversations(user.id)} className="btn-outline py-2 px-4 text-sm">↻</button>
      </div>
      {loading ? (
        <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-t-transparent rounded-full animate-spin" style={{borderColor:'#1A73E8',borderTopColor:'transparent'}} /></div>
        ...
        <div className="space-y-2">
          {conversations.map(conv => (
            <button key={conv.missionId}
              onClick={() => router.push(`/provider/chat/${conv.missionId}`)}
              className="card w-full text-left flex items-center gap-4 hover:shadow-md transition-shadow">
              <div className="w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center text-white font-semibold" style={{background:'#1A73E8'}}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {conv.lastMessage && (
                    <p className="text-xs text-gray-400 ml-2">
                      {formatDistanceToNow(new Date(conv.lastMessage.created_at), { locale: DATE_LOCALES[lang] ?? enUS, addSuffix: true })}
                    </p>
                  )}
                </div>
              ...
              {conv.unreadCount > 0 && (
                <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-white text-xs font-bold" style={{background:'#1A73E8'}}>
                  {conv.unreadCount}
                </div>
              )}
            </button>
