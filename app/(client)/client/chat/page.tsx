...
export default function ClientChatList() {
  ...
  const loadConversations = async (uid: string) => {
    ...
    // Join diretto su users tramite FK — evita query separate bloccate da RLS
    ...
    const convs = await Promise.all(missions.map(async (m) => {
      // Usa i dati joinati invece di query separate
      const otherUser = uid === m.client_id
        ? (m.provider as any)
        : (m.client as any)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">
          💬 {t('navChat')} {totalUnread > 0 && <span className="text-accent text-xl">({totalUnread})</span>}
        </h1>
        <button onClick={() => user && loadConversations(user.id)} className="btn-outline py-2 px-4 text-sm">↻ {t('chatRefreshButton')}</button>
      </div>
        ...
        <div className="space-y-2">
          {conversations.map(conv => (
            <button key={conv.missionId}
              onClick={() => router.push(`/client/chat/${conv.missionId}`)}
              className="card w-full text-left flex items-center gap-4 hover:shadow-md transition-shadow">
              <div className="w-12 h-12 bg-accent rounded-full flex-shrink-0 flex items-center justify-center text-white font-semibold">
