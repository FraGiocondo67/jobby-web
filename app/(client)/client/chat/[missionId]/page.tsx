...
export default function ChatDetail() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      setUser(u)
      if (u && missionId) {
        // Carica info missione
        const { data: mission } = await supabase.from('missions')
          .select('id, title, client_id, provider_id, category:service_categories(icon)')
        ...
        // Realtime
        channelRef.current = supabase.channel(`chat:${missionId}`)
  ...
  const sendMessage = async () => {
    ...
    try {
      await supabase.from('messages').insert({
        ...
        receiver_id: conv.otherUser?.id, content: text,
      })
    } catch { setInput(text) }
    ...
    <div className="flex flex-col h-[calc(100vh-5rem)] -my-8 -mx-4 sm:-mx-6 lg:-mx-8">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto bg-gray-50 p-4 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-sm">Nessun messaggio ancora. Scrivi per primo!</p>
          </div>
        ) : (
          messages.map((msg, i) => {
            const isMe = msg.sender_id === user?.id
            const prevMsg = messages[i - 1]
            const showTime = !prevMsg || fmtTime(msg.created_at) !== fmtTime(prevMsg.created_at)
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className="max-w-[75%]">
                  <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed
                    ${isMe ? 'bg-accent text-white rounded-br-sm' : 'bg-white text-gray-900 shadow-sm rounded-bl-sm'}`}>
                    {msg.content}
                  </div>
                  {showTime && (
      ...
      <div className="bg-white border-t border-gray-100 p-4">
        <div className="flex items-end gap-3">
          <textarea
            className="flex-1 resize-none border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent max-h-32"
            rows={1} placeholder="Scrivi un messaggio..." value={input}
            onChange={e => setInput(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage() } }}
          />
          <button onClick={sendMessage} disabled={!input.trim() || sending}
            className={`p-3 rounded-xl transition-colors ${input.trim() && !sending ? 'bg-accent text-white hover:bg-orange-600' : 'bg-gray-100 text-gray-400'}`}>
            <Send size={18} />
          </button>
        </div>
      </div>
