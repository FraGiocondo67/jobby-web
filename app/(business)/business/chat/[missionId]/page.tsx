...
export default function BusinessChatDetail() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (u && missionId) {
        ...
        if (mission) {
          const { data: otherUser } = await supabase.from('users').select('id, full_name').eq('id', mission.client_id).maybeSingle()
          setConv({ ...mission, otherUser })
        }
        const { data: msgs } = await supabase.from('messages').select('*').eq('mission_id', missionId).order('created_at', { ascending: true })
        setMessages(msgs ?? [])
        setLoading(false)
        await supabase.from('messages').update({ read_at: new Date().toISOString() })
          .eq('mission_id', missionId).eq('receiver_id', u.id).is('read_at', null)
        channelRef.current = supabase.channel(`chat-b:${missionId}`)
          .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `mission_id=eq.${missionId}` }, (payload) => {
  ...
  const sendMessage = async () => {
    ...
    try { await supabase.from('messages').insert({ mission_id: missionId, sender_id: user.id, receiver_id: conv.otherUser?.id, content: text }) }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-5rem)] -my-8 -mx-4 sm:-mx-6 lg:-mx-8">
      <div className="bg-white border-b px-4 py-3 flex items-center gap-3">
        ...
        <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm" style={{background: '#5B2D8E'}}>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto bg-gray-50 p-4 space-y-3">
        {loading ? <div className="flex justify-center py-8"><div className="w-6 h-6 border-4 border-t-transparent rounded-full animate-spin" style={{borderColor: '#5B2D8E', borderTopColor: 'transparent'}} /></div>
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className="max-w-[75%]">
                  <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${isMe ? 'text-white rounded-br-sm' : 'bg-white text-gray-900 shadow-sm rounded-bl-sm'}`}
                    style={isMe ? {background: '#5B2D8E'} : {}}>
                    {msg.content}
                  </div>
      ...
      <div className="bg-white border-t p-4">
        <div className="flex items-end gap-3">
          <textarea className="flex-1 resize-none border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 focus:border-purple-400 max-h-32"
            rows={1} placeholder="Scrivi..." value={input} onChange={e => setInput(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage() } }} />
          <button onClick={sendMessage} disabled={!input.trim() || sending}
            className={`p-3 rounded-xl ${input.trim() && !sending ? 'text-white hover:opacity-90' : 'bg-gray-100 text-gray-400'}`}
            style={input.trim() && !sending ? {background: '#5B2D8E'} : {}}>
            <Send size={18} />
          </button>
        </div>
      </div>
